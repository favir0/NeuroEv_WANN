...
class Species:
    ...
    def age_fitness_adjustment(self):
        if self.age < config.young_age:
            self.avg_adjusted_fitness *= config.young_multiplier
        if self.age > config.old_age:
            self.avg_adjusted_fitness *= config.old_multiplier
